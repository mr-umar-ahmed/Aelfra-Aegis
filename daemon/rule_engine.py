#!/usr/bin/env python3
"""
Aelfra Aegis Policy Rule Engine (Industry Upgrade 1)
Supports configurable JSON-based detection rules, hot-reloading watchdog,
single-event condition evaluation, and multi-stage temporal chain correlation.
"""
...
class RuleEngine:
    def __init__(self, rules_path: str):
        ...
        # Start hot-reload watchdog thread (checks mtime every 5s)
    ...
    def load_rules(self) -> bool:
        """Loads and parses detection rules from the JSON configuration file."""
        if not os.path.exists(self.rules_path):
            print(f"[RULE_ENGINE WARNING] Rules file not found at {self.rules_path}", flush=True)
            return False
        ...
        try:
            ...
            print(
                f"[RULE_ENGINE] Successfully loaded {len(parsed_rules)} detection rules from {os.path.basename(self.rules_path)} (v{data.get('version', '1.0')})",
                flush=True,
            )
            return True
    ...
    def _check_conditions(self, event: Dict[str, Any], conditions: Dict[str, Any]) -> bool:
        ...
        if "parent_comm_in" in conditions:
            ...
            # If parent_comm is explicitly provided, check membership
            if parent_comm:
                if parent_comm not in allowed_parents:
                    return False
            else:
                # If comm itself is in allowed_parents (e.g. node running scripts) or ppid exists
                comm = (event.get("comm") or "").lower()
        ...
        if conditions.get("exclude_private_ips", False):
            ...
            try:
                ...
                # Non-IP string (e.g. loopback alias)
    ...
    def evaluate_event(self, event: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        Returns list of matched rule objects enriched with rule_id, severity, confidence, and action.
        """
        matched = []
        ...
        for rule in active_rules:
            if rule.get("event_type") == "chain":
                continue  # Chain rules are evaluated across time windows in evaluate_chain
    ...
    def evaluate_chain(self, pid_events: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        # First, evaluate each individual event in pid_events to find triggered rule IDs
        ...
        for chain_rule in chain_rules:
            ...
            # Check if all required rule IDs in the sequence exist in this PID's event history
            if not all(req_id in all_triggered_ids for req_id in required_sequence):
                continue

            # Check temporal sequence ordering within time window
            timestamps = []
            found_sequence = True
            current_time = 0.0
